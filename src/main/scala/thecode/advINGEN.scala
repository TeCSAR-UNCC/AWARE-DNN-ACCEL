...
class advINGEN (val WaitTime: Long,val ChanPar: Int,val waitmult: Int) extends Module 
{
	val io = IO(new Bundle {
		...
		val Delay = RegInit(0.U(log2Ceil(WaitTime*waitmult+1).W))//internal state reg
		val limitREG =RegInit(1.U(1.W))
		val ValidREG =RegInit(1.U(1.W))
		val Sum = RegInit(0.U(log2Ceil(256).W))//internal state reg
		when (Delay < ((WaitTime*waitmult).asUInt(log2Ceil(WaitTime*waitmult+1).W))) 
		{ 
			Delay:= Delay + 1.U
			limitREG :=0.U
				
		}
		when (Delay === ((WaitTime*waitmult-1).asUInt(log2Ceil(WaitTime*waitmult+1).W)))
		{
			limitREG:= 1.U
